...
class Predict:
    ...
    def predict(self):
        ...
        for i in range(len(X)):
            # Prétraitement de l'image pour le produit i
            image = self.preprocess_image(X["image_path"].iloc[i], target_size)
            image = tf.convert_to_tensor([image], dtype=tf.float32)  # Ajouter une dimension pour le batch

            # Prédiction RNN pour le produit i
            sequence = padded_sequences[i:i+1]  # Ajouter une dimension pour le batch
            rnn_prediction = self.rnn.predict(sequence)
            rnn_proba.append(rnn_prediction)

            # Prédiction VGG16 pour le produit i
            vgg16_prediction = self.vgg16.predict(image)
            vgg16_proba.append(vgg16_prediction)
        
        # Convertir les listes en tableaux NumPy
        rnn_proba = np.squeeze(np.array(rnn_proba))
        vgg16_proba = np.squeeze(np.array(vgg16_proba))
