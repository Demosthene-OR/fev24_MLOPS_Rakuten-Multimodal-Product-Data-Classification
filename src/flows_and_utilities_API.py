...
def new_product_proposal(input_data: NewProductsProposalInput, token: Optional[str] = Depends(oauth2_scheme)):
    ...
    try:
        if os.path.exists(reserve_path):
            reserve_df = pd.read_csv(reserve_path)
...
def  add_new_products(input_data: NewProductsInput, token: Optional[str] = Depends(oauth2_scheme)):
    ...
    try:
        ...
        if os.path.exists(reserve_path):
            reserve_df = pd.read_csv(reserve_path)
            # X_new_df = pd.read_csv(X_new_path)
            reserve_df = reserve_df[~reserve_df.isin(X_new_df)].dropna()
            reserve_df.to_csv(reserve_path, index=False)
            
        # Delete specific files in data/predict
        files_to_delete = ["new_classes.csv", "predictions.csv", "X_test_update.csv"]
        for file_name in files_to_delete:
